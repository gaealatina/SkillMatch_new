...
function getRecentActivity(user) {
  ...
  user.skills.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 2)
    .forEach(skill => {
      activities.push({
        title: `${skill.name} Progress`,
        desc: `Reached ${skill.level}% proficiency`,
  ...
  user.projectHistory.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 1)
    .forEach(project => {
      activities.push({
  ...
  if (user.recommendations.length > 0 && activities.length < 3) {
    activities.push({
      title: 'New Recommendations',
      desc: `${user.recommendations.length} skills suggested`,
      time: 'Recently',
      icon: 'Users'
    });
  }
