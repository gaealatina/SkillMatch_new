...
export default function Profile() {
  ...
  const handleDeleteSkill = async (skillId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_BASE_URL}/profile/skills/${skillId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAddProject = async (projectData) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${API_BASE_URL}/profile/projects`, projectData, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setProjectHistory([...projectHistory, response.data.project]);

      // Refresh suggestions if project addition affects recommendations
      const suggestionsResponse = await axios.get(SUGGESTIONS_URL, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setRecommendations(suggestionsResponse.data.recommendations || []);
...
function GrowthPlanCard({ recommendation }) {
  ...
  return (
    <div className="border border-border rounded-lg hover:shadow-md transition-all duration-200">
      <div className="p-6">
        {/* Skill Header */}
        <div className="mb-6">
          <h3 className="text-xl font-bold text-card-foreground mb-2">{recommendation.skillName}</h3>
          {recommendation.currentLevel > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">Current Level: {recommendation.currentLevel}%</span>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          <h4 className="font-semibold text-card-foreground mb-3">Why this matters:</h4>
          <div className="bg-muted/30 rounded-lg p-4 border border-border">
            <p className="text-card-foreground leading-relaxed">{recommendation.reason}</p>
          </div>
        </div>

        {/* Suggested action section */}
        <div className="mb-6">
          <h4 className="font-semibold text-card-foreground mb-3">Suggested action:</h4>
          <div className="bg-muted/30 rounded-lg p-4 border border-border">
            <p className="text-card-foreground leading-relaxed">{recommendation.suggestedAction}</p>
          </div>
        </div>

        {/* Resources section */}
        <div>
          <h4 className="font-semibold text-card-foreground mb-3">Resources:</h4>
            ...
            {recommendation.resourceLinks &&
              recommendation.resourceLinks.map((link, index) => (
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 text-sm font-medium text-card-foreground bg-card border border-border rounded-lg hover:bg-muted hover:shadow-md transition-all duration-200"
                >
                  {link.title}
                </button>
              ))}
